...
import data_helpers_ch_all as data_helpers
...
def train():
    with tf.device('/cpu:0'):
        pos1, pos2, all_words, y = data_helpers.load_data_and_labels(FLAGS.train_dir,FLAGS.max_sentence_length,mode='train')


    x = []
    for idx in tqdm(range(len(pos1))):
        all_words_list = all_words[idx].reshape(-1,).tolist()
        x.append([pos1[idx]] + [pos2[idx]] + all_words_list)
    ...
    num_round = 20000
    bst = xgb.train(params, d_train, num_round, watchlist, early_stopping_rounds=1)

    
    ## GGGGGGGGGGGGGGGGGGGGGGGGGGG
    pos1, pos2, all_words, y = data_helpers.load_data_and_labels(FLAGS.eval_dir,FLAGS.max_sentence_length,mode='eval')

    ### GGGGGGGGGGGGGGGGGGGGGGGGGGGG
    x_eval = []
    for idx in tqdm(range(len(pos1))):
        all_words_list = all_words[idx].reshape(-1,).tolist()
        x_eval.append([pos1[idx]] + [pos2[idx]] + all_words_list)
