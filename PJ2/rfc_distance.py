...
def train():
    with tf.device('/cpu:0'):
        e1_list, e2_list, pos1, pos2, distance, POS_tag_e1, POS_tag_e2, between, before, after, y = data_helpers.load_data_and_labels(FLAGS.train_dir, mode='train')
    ...
    for idx in range(len(e1_list)):
        ...
        x.append(e1_list[idx]+e2_list[idx]+[pos1[idx]]+[pos2[idx]]+[distance[idx]]+POS_tag_e1[idx]+POS_tag_e2[idx]+between_flatten+before_flatten+after_flatten)
    ...
    with tf.device('/cpu:0'):
        e1_list, e2_list, pos1, pos2, distance, POS_tag_e1, POS_tag_e2, between, before, after, y = data_helpers.load_data_and_labels(FLAGS.eval_dir, mode='eval')
    ...
    for idx in range(len(e1_list)):
        ...
        before_flatten = before[idx].reshape(-1,).tolist()
        after_flatten = after[idx].reshape(-1,).tolist()        
        x_eval.append(e1_list[idx]+e2_list[idx]+[pos1[idx]]+[pos2[idx]]+[distance[idx]]+POS_tag_e1[idx]+POS_tag_e2[idx]+between_flatten+before_flatten+after_flatten)
    ...
    with open("ans_rfc_distance.txt", "w") as fout:
