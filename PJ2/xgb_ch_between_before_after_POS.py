...
def train():
    with tf.device('/cpu:0'):
        e1_list, e2_list, pos1, pos2, partofspeech_e1, partofspeech_e2, between, before, after, y = data_helpers.load_data_and_labels(FLAGS.train_dir,FLAGS.max_sentence_length,mode='train')
    ...
    for idx in range(len(e1_list)):
        ...
        x.append(e1_list[idx] + e2_list[idx] + [pos1[idx]] + [pos2[idx]] + partofspeech_e1[idx] 
    ...
    e1_list, e2_list, pos1, pos2, partofspeech_e1, partofspeech_e2, between, before, after, y = data_helpers.load_data_and_labels(FLAGS.eval_dir,FLAGS.max_sentence_length,mode='eval')
    ...
    for idx in range(len(e1_list)):
        ...
        before_flatten = before[idx].reshape(-1,).tolist()
        after_flatten = after[idx].reshape(-1,).tolist()
        x_eval.append(e1_list[idx] + e2_list[idx] + [pos1[idx]] + [pos2[idx]] + partofspeech_e1[idx] 
                  + partofspeech_e2[idx] + between_flatten + before_flatten + after_flatten)
    ...
    with open('./ans/ans_xgb_between_before_after_POS.txt','w') as f:
