...
def train():
    with tf.device('/cpu:0'):
        e1_list, e2_list, pos1, pos2, y = data_helpers.load_data_and_labels(FLAGS.train_dir, mode='train')
    ...
    for idx in range(len(e1_list)):
        x.append(e1_list[idx]+e2_list[idx]+[pos1[idx]]+[pos2[idx]])
    ...
    with tf.device('/cpu:0'):
        e1_list, e2_list, pos1, pos2, y = data_helpers.load_data_and_labels(FLAGS.eval_dir, mode='eval')
    ...
    x_eval = []
    for idx in range(len(e1_list)):
        x_eval.append(e1_list[idx]+e2_list[idx]+[pos1[idx]]+[pos2[idx]])
    ...
    with open("ans_rfc.txt", "w") as fout:
