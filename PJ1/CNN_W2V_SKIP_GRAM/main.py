...
from keras.layers import Input, GRU, LSTM, Dense, Dropout, Bidirectional, Conv2D, Flatten, Reshape, MaxPooling2D
...
parser.add_argument('-emb_dim', '--embedding_dim', default=50, type=int)
...
def to_word2vec(token_corpus):
    ...
    vector_size = 50
    window_size = 5
    word2vec = Word2Vec(sentences=token_corpus, min_count=1, size=vector_size,  window=window_size, negative=20, iter=10, sg=1)
    print("pretraining word2vec is over!!")
    return word2vec

# build model
def simpleRNN(args):
    inputs = Input(shape=(args.max_length,50))
    ...
    """
    # Reshape
    Reshape_embeddings = Reshape((50, 50, 1), trainable=True)(inputs)
    # CNN
    CNN_embeddings = Conv2D(32, (5, 5))(Reshape_embeddings)
    CNN_embeddings_2 = Conv2D(16, (5, 5))(CNN_embeddings)

    #Maxpooling_embeddings = MaxPooling2D(pool_size=(2, 2))(CNN_embeddings)
    # Flatten
    Flatten_embeddings = Flatten()(CNN_embeddings_2)
    # DNN layer
    outputs = Dense(args.hidden_size//2, 
                    activation='tanh')(Flatten_embeddings)
